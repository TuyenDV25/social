...
public interface FriendRepository extends JpaRepository<Friend, Long> {
	...
	@Query(value = "select fr.id, fr.user_info_id as userInfoId, fr.created_date as createdDate from friend fr join user_info_list_friend u_fr on fr.id=u_fr.list_friend_id join user_info u on u.id = u_fr.user_info_id and u_fr.user_info_id=?1 order by fr.id desc",
	...
	@Query(value = "select count (*) from friend fr where fr.user_info_id = ?1 and fr.created_date between ?2 and ?3 ",
			nativeQuery = true)
	Long countNewFriends(Long userInfoId, Date from, Date to);
